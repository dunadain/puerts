...
program
  ...
  .addOption(
    new Option(
      "--arch <arch>",
      "the target architecture. 'auto' means build all available archs for the platform and universal binary will be created in osx."
    )
      .default("auto")
  ...
  .addOption(
    new Option(
      ...
      "Debug ver or Release ver. In Windows, Debug means DebugWithRelInfo"
    )
      .default("Release")
      .choices(["Release", "Debug"])
  )
  .addOption(
    new Option("--path <exportpath>", "the path to export").default("")
  )
  ...
  .action(function (quickcommand, options) {
    ...
    if (quickcommand) {
      ...
      console.log("quick command parse result:", {
        backend,
        config,
        arch,
        platform,
    ...
    if (options.platform && options.arch == "auto") {
      ...
      if (options.platform == "osx") {
        ...
        promiseChain = promiseChain.then(() =>
          makeOSXUniveralBinary(cwd, outputs)
        );
      }
...
const backendProgram = program.command("backend");

backendProgram.command("download [backend] [url]").action((backend, url) => {
  downloadBackend(cwd, backend, url);
});
backendProgram.command("clean").action(() => {
